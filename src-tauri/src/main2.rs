...
struct FileMonitorState {
    ...
    current_offset: u64, // Добавляем сохранение позиции
}
...
fn start_file_loading(
    ...
    let start_offset = {
        ...
        if mon.current_file.as_deref() == Some(&file_path) {
            mon.current_offset
        } else {
    ...
    // Создаём токен отмены и сохраняем в state
    ...
    // Клонируем всё, что нам нужно в поток
...
async fn get_system_info(
    ...
    let cpu_temp = match state.stats.cpu_temp() {
        Ok(temp) => Some(temp),
        Err(_) => None,
    };
...
fn set_current_file(path: String, state: State<'_, MonitoringState>) {
    ...
    // При смене файла сбрасываем позицию
...
fn start_file_monitoring(
    ...
    thread::spawn(move || {
        ...
        let mut file = match File::open(&path) {
            Ok(f) => f,
            Err(e) => {
                let _ = app_handle_clone.emit("monitoring_error", format!("Failed to open file: {}", e));
                return;
            }
        };
        ...
        if let Err(e) = file.seek(SeekFrom::End(0)) {
            let _ = app_handle_clone.emit("monitoring_error", format!("Failed to seek file: {}", e));
            return;
        }
        ...
        loop {
            let should_continue = {
                let monitor = state_clone.lock().unwrap();
                monitor.is_running && monitor.current_file.as_ref() == Some(&file_path)
            };
            if !should_continue {
                println!("[MONITOR] Stopping monitoring for: {}", file_path);
                break;
            }
            ...
            let metadata = match path.metadata() {
                Ok(m) => m,
                Err(e) => {
                    let _ = app_handle_clone.emit("monitoring_error", format!("Failed to get metadata: {}", e));
            ...
            if metadata.len() < last_size {
                println!("[MONITOR] File was truncated, resetting position");
                offset = 0;
                file.seek(SeekFrom::Start(0)).ok();
                {
                    let mut monitor = state_clone.lock().unwrap();
                    monitor.current_offset = 0;
                }
                let _ = app_handle_clone.emit("file_cleared", ());
            }
                            ...
                            if bytes_read > 0 {
                                ...
                                for line in content.lines() {
                                    ...
                                    if !trimmed.is_empty() {
                                        ...
                                        if batch.len() >= BATCH_SIZE {
                                            let _ = app_handle_clone.emit("new_logs_batch", batch.clone());
                                            batch.clear();
                                        }
                                    }
                                }
                                if !batch.is_empty() {
                                    let _ = app_handle_clone.emit("new_logs_batch", batch);
                                }
                            }
                        }
                    }
